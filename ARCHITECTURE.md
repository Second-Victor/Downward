...
`MarkdownSyntaxScanner` is the first extracted recognition boundary. It is UIKit-free and currently returns line ranges, indented and fenced code block ranges, merged protected code ranges, inline code spans, and protected image ranges. `MarkdownStyledTextRenderer` consumes those scanner results and still coordinates the remaining regex-driven recognition, rendering order, and TextKit handoff. `MarkdownSyntaxStyleApplicator` owns the extracted attributed-string mutation slice, while broader theme role tables and future semantic parsing remain incomplete. `MarkdownSyntaxVisibilityPolicy` owns the pure decision for whether a syntax token should be hidden for a given mode and revealed range; TextKit still applies and draws the resulting attributes.
